...
import sys, random, hashlib, sqlite3, os
import os_win, ctl_scan, ora_link1
...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "Oracle 控制文件工具  V%s" % self.version))
        ...
        abort = 'Oracle 控制文件工具  V%s\n\nOracle 9i,10g,11g,12c各版本\n支持扫描大小端平台的文件\n支持多种智能拼接算法\n此版本只能运行在64位Windows系统中\n可扫描镜像文件和磁盘.扫描磁盘需以管理员运行.\n此程序注册单次有效,程序重启需重新获取注册码注册' % self.version
    ...
    def disk_open(self):
        ...
        self.disk_info = os.disk()
        self.disk_sum = len(self.disk_info)
        self.comboBox.addItem('')
        self.comboBox.addItem('------------------磁盘-------------------')
        for i in range(len(self.disk_info)):
            self.comboBox.addItem(
                self.disk_info[i].disk_caption + '[%5.1fG]' % (self.disk_info[i].disk_size / 1024.0 / 1024 / 1024))
    ...
    def scan_start(self):
        ...
        page_size = int(self.comboBox_5.currentText())  # 页面大小
        endian = endian_0[self.comboBox_6.currentText()]  # 大小端

        self.textEdit.append('file: %s' % self.f_name)
        out_path = QtWidgets.QFileDialog.getExistingDirectory(self, "选择一个存放中间数据(<4G)的目录...", None)  # 选择文件夹对话框
        if out_path == '':
            return
        db_name = out_path + '/ctl_scan.db'
        self.textEdit.append('scan db: %s' % db_name)
        ...
        file_infos = ctl_scan.ora_scan(self.f_name, db_name, start_off, end_off, endian, scan_size)  # 页面扫描 *****
        ora_link1.link_wl(self.f_name, db_name)  # 物理拼接

        # self.textEdit.append('db name: %s'%file_infos[0].SID)
        # self.textEdit.append('db version: %s'%file_infos[0].version)
        ...
        self.show_tab1(db_name)

    # 物理碎片列表
    def show_tab1(self, db_name):
        conn = sqlite3.connect(db_name)
        cursor = conn.cursor()
        cursor.execute("select * from link_1 ;")
    ...
    def data_out(self):
        ...
        self.out_widget.setWindowTitle('导出设置')
        ...
        self.label_13.setGeometry(QtCore.QRect(10, 140, 50, 23))
        self.label_13.setText('out_sql')
        self.label_14 = QtWidgets.QLabel(self.out_widget)
        self.label_14.setGeometry(QtCore.QRect(10, 200, 200, 23))
        ...
        self.lineEdit_13.setGeometry(QtCore.QRect(80, 140, 400, 23))
        ...
        self.pushButton_14.setGeometry(QtCore.QRect(80, 260, 160, 23))
        self.pushButton_14.setText('设置完成，开始导出')
        self.pushButton_15 = QtWidgets.QPushButton(self.out_widget)  # 取消导出
        self.pushButton_15.setGeometry(QtCore.QRect(300, 260, 100, 23))
        self.pushButton_15.setText('取消导出')
    ...
    def fileOpen1(self, ):
        fn, _ = QtWidgets.QFileDialog.getOpenFileName(self, "Open ", None, "All Files (*)")
        self.lineEdit_10.setText(fn)

    def fileOpen2(self, ):
        fn, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Save File ... ", None, "All Files (*)")
    ...
    def data_out1(self):
        ...
        if file_in != '' and file_out != '' and db_name != '' and out_sql != '':
            ...
            QtWidgets.QMessageBox.about(self, "About", "参数不全,请输入完整参数 ...  \t\n")
