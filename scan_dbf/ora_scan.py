...
class Scan_1(QThread):  # 类Scan_1继承自 QThread
    ...
    def ora_scan(self, f_name, db_name, logging, start_off, end_off, endian, page_size, scan_size):
        ...
        for i in range(0, loop_1 + 1):  # 处理文件的所有buffer块
            ...
            if (i < loop_1):  # 处理文件的所有buffer块
                for ii in range(0, loop_2):
                    ...
                    #   fmt = fmt_1 + '2BHI8B2H2I'
                    fmt = fmt_1 + '2BHIIH2B2H2I'
                    data2 = s(fmt, data1[0:28])
                    flag = data2[7]
                    ...
                    if chk2 == 1:  # 页头校验通过
                        ...
                        if chk3 == 1 and flag & 4 == 4:  # 通过页尾校验，并且开启了页面校验和
                            chk1 = ora_check.ora_page_check_1(data1, fmt_1)  # 异或校验
                        elif chk3 == 1 and flag & 4 == 0 and data2[8] == 0:  # 通过页尾校验，并且没有开启页面校验和
                            chk1 = 1  # 异或校验不做
                        else:
                            chk1 = 0
                        if chk1 == 1:
                            ...
                            scn_1 = data2[4] + data2[5] * 4294967296
                            col_sum = 0
                            if data2[0] == 0:
                                page_0 += 1
                            elif data2[0] == 6:
                                page_6 += 1
                                if data2[10] == 1 and file > 3:
                                    col_sum = self.get_col_sum(data1, fmt_1)  # 获取数据页中记录的总列数
                                # print(page, col_sum)
                            elif data2[0] == 17:
                                page_17 += 1
                            elif data2[0] == 11:
                                if page != 1:
                                    continue
                                page_11 += 1
                                file_info, boot = ora_check.file_blk_1(data1)
                                col_sum = file_info.blk_sum
                                if file_info.ts_name == 'TEMP':
                                    continue
                                print('id:%d, off:%d, ' % (page_11, offset), end='')
                                print(file_info.info)
                                logging.info('id:%d, off:%d, %s' % (page_11, offset, file_info.info))
                                self.file_infos.append(file_info)
                            sum += 1
                            ii += page_size // scan_size
                            cursor.execute(
                                "insert into ora_page(offset,page_type,obj_id,scn,page_no,file_no,col_sum) values(?,?,?,?,?,?,?)",
                                (offset, data2[0], data2[11], scn_1, page, file, col_sum))
                            if sum % 100000 == 0:
                ...
                for ii in range(0, loop_2_1):
                    ...
                    data2 = s(fmt, data1[0:28])
                    flag = data2[7]
                    chk2 = ora_check.ora_page_check_2(data1, fmt_1)
                    if chk2 == 1:
                        ...
                        if chk1 == 1:
                            ...
                            if data2[0] == 0:
                                ...
                                file_info, boot = ora_check.file_blk_1(data1)
                                col_sum = file_info.blk_sum
                                if file_info.ts_name == 'TEMP':  # 过滤掉temp文件的block#1
                                    continue
                                print('id:%, off:%d ' % (page_11, offset), end='')
                                print(file_info.info)
                                logging.info('id:%d, off:%d, %s' % (page_11, offset, file_info.info))
                                self.file_infos.append(file_info)
                            sum += 1
        ...
        print("\n总页数:%d, 空页数:%d, 数据页:%d, 段头页:%d, 文件头:%d" % (sum, page_0, page_6, page_17, page_11))  # 总页数,空页数,数据页数
        ...
        logging.info("\n总页数:%d, 空页数:%d, 数据页:%d, 段头页:%d, 文件头:%d" % (sum, page_0, page_6, page_17, page_11))
